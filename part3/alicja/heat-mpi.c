...
int main( int argc, char *argv[] )
{
    ...
    // TODO compute number of rows per process
    ...
    for (int i=0; i<numprocs; i++) {
        if (i>0) {
                ...
                MPI_Send(&param.u[i*(rows_per_proc*np)], (rows_per_proc+2)*np, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
                MPI_Send(&param.uhelp[i*(rows_per_proc*np)], (rows_per_proc+2)*np, MPI_DOUBLE, i, 0, MPI_COMM_WORLD);
        }
    }
	...
	switch( param.algorithm ) {
	    case 0: // JACOBI
            ...
            MPI_Recv(&param.u[(rows_per_proc*np)+np], np, MPI_DOUBLE, myid+1, iter, MPI_COMM_WORLD, &status);
        ...
        // TODO compute the current value of global residual
    ...
    for (int i=1; i<numprocs; i++)
        MPI_Irecv(&param.u[(rows_per_proc*i+1)*np], rows_per_proc*np, MPI_DOUBLE, i, 0, MPI_COMM_WORLD, &request[i-1]);
    ...
    rows_per_proc = columns / numprocs;
    np = columns + 2;

    // TODO reduce memory allocated to each worker to fit only one segment with ghost points
    // allocate memory for worker
    double * u = calloc( sizeof(double),(rows_per_proc+2)*(columns+2) );
    double * uhelp = calloc( sizeof(double),(rows_per_proc+2)*(columns+2) );
    if( (!u) || (!uhelp) )
    {
	...
	switch( algorithm ) {
	    case 0: // JACOBI
	        ...
	        if (myid < numprocs-1)
            MPI_Isend(&u[rows_per_proc*np], np, MPI_DOUBLE, myid+1, 0, MPI_COMM_WORLD, &r[2]);

            // TODO send the first row (of internal points) to the previous process
            MPI_Isend(&u[np], np, MPI_DOUBLE, myid-1, 0, MPI_COMM_WORLD, &r[0]);

		    // TODO receive the last row from the previous process and store it in the first row as ghost points (after the first iteration of computing values, because ghost points haven't changed yet)
		    MPI_Irecv(&u[0], np, MPI_DOUBLE, myid-1, 0, MPI_COMM_WORLD, &r[1]);

            if (myid < numprocs-1)
            // TODO receive the first row from the next process and store it in the last row as ghost points
            MPI_Irecv(&u[(rows_per_proc*np)+np], np, MPI_DOUBLE, myid+1, 0, MPI_COMM_WORLD, &r[3]);

            // TODO wait until the exchange of the first row is complete (r[0] and r[1])
            MPI_Waitall(2, r, MPI_STATUS_IGNORE); // TODO does it makes sense to use non-blocking here?
    ...
    MPI_Send(&u[np], rows_per_proc*np, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
